...
The page SHALL provide an "Editar valores" control that reveals inputs for Simples rate (%), INSS (R$) and accounting fee (R$). Changing any of them SHALL recalculate immediately. Edited values SHALL persist on the same device across visits, and a "Restaurar padrão" control SHALL return all three to the defaults.

#### Scenario: One-off different fee
- **WHEN** the user opens "Editar valores" and sets the accounting fee to `350,00`
